...
router.post('/getAllCourseReviewes', function(req, res, next) {
    coursId = req.body.id
    Courses.findOne({_id:coursId}).select('reviews -_id').populate({path:'reviews'})
        .then(courses=>{
        //  let a =   courses.reviews.map((review)=>{
        //       return (review.visibility==="true") ? review  : 1 
                
        //     })
        //     console.log(a);
        if (true) {
            for (let index = 0; index < courses.length; index++) {
                courses.reviews[index].visibility=="true"
                }
    //   fillter 
            //    console.log(index)
